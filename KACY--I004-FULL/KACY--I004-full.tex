...
%	\mainmatter
	...
	\fancyhead[LE,RO]{Olympiad Algebra (Vol. I):\hfill 1220 Problems\hfill \texttt{KACY--I004} Booklet}
...
\newpage
	...
	\begin{tcolorbox}
		\begin{question}
			The quartic equation $x^4-5x^3+2x^2+20x-24=0$
	...
	\begin{tcolorbox}
		...
		\begin{question}
			...
			has been written as $P(x)=(x+a-1)(x+a+1)(x^2+b)(x^2-ax+7b)$,
	...
	\begin{tcolorbox}
		...
		\begin{question}
			...
			\begin{align*}
				f(x)= \frac{1}{1-x},
			\end{align*}
			Which one is $f(f(f(x)))$?
			
			\vspace{0.5em}
			
			$\textbf{(A)}~\dfrac{1}{x}\qquad\textbf{(B)}~\dfrac{1}{x-1}\qquad\textbf{(C)}~x\qquad\textbf{(D)}~x-1\qquad\textbf{(E)}~\dfrac{x-1}{x}$	
		\end{question}
	\end{tcolorbox}
	
	\begin{solution}%[name=Solution by Parviz Shahriari]
		The answer is $f(f(f(x)))=x$. It is easy to see that $f(f(x))=\frac{x-1}{x}$, and applying $f$ one more time results in  $f(f(f(x)))=\boxed{x \ \textbf{(C)}}$.
	\end{solution}
